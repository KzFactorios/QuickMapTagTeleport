  ...
  if is_favorite then
    ...
    if not existing_fave then
      ...
      if new_index ~= -1 then
        ...
        -- TODO make 10 max faves a setting
...
function map_tag_utils.save_tag(player, position, text, icon, display_text, description, is_favorite)
  if not player then return end
  ...
  wutils.establish_working_qmtt(working_tag, player, is_favorite, display_text, description)

  -- Deal with faves, note that we have already updated tags and qmtts
  map_tag_utils.curate_player_fave_places(is_favorite, player, wutils.format_idx_from_position(position))
